...
void  SceneMain::Update(double dt)
{
	...
	//RenderTextOnScreen(meshList[GEO_TEXT], "PLAY", text_color, 4, 30, 25);
	if (posX > 29.5 && posX < 44.5)
	{
		if (posY > 24.5 && posY < 29.5)
		{
...
void SceneMain::RenderMeshOnScreen(Mesh* mesh, int x, int y, float
	sizex, float sizey)
{
...
void SceneMain::RenderPhysicalWorld()
{
	...
	//RENDER THE CAMERA AND PHYSICAL OBJECTS
	{
		...
		modelStack.PushMatrix();
		modelStack.Translate(light[0].position.x, light[0].position.y + 40,
			light[0].position.z);
...
{
	RenderTextOnScreen(meshList[GEO_TEXT], "Life of a Scammer", Color(1, 0, 0), 3, 15, 45);
	RenderTextOnScreen(meshList[GEO_TEXT], "Scam for 5 days...", Color(1, 0, 0), 3, 15, 40);
	RenderTextOnScreen(meshList[GEO_TEXT], "PLAY", text_color, 4, 30, 25);
}
